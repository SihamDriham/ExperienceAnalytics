...
def analyze_sentiment_tech_correlation(df_users, df_apps, df_devices, df_sentiments):
    ...
    df_join1 = s.join(
        ...
        (to_date(col("s.record_date")) == to_date(col("d.record_date"))),
        how="inner"
    )
    ...
    df_join2 = df_join1.join(
        ...
        (to_date(col("s.record_date")) == to_date(col("a.record_date"))),
        how="inner"
    )
...
def compute_digital_sentiment_score(df_users, df_devices, df_sentiments):
    ...
    df_join_user = df_sentiments.alias("s").join(
        df_users.alias("u"),
        (col("s.user_id") == col("u.id")) & (to_date(col("s.record_date")) == to_date(col("u.record_date"))),
        how="inner"
    )
    ...
    df_full = df_join_user.join(
        df_devices.alias("d"),
        (col("s.user_id") == col("d.user_id")) & (to_date(col("s.record_date")) == to_date(col("d.record_date"))),
        how="inner"
    )
